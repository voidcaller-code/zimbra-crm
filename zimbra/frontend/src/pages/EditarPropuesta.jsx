...
const EditarPropuesta = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        const [propuestaResponse, prospectosResponse] = await Promise.all([
          api.get(`/propuestas/${id}/`),
          api.get('/prospectos/'),
        ]);

        const propuesta = propuestaResponse.data;

        setForm({
          prospecto: propuesta.prospecto || '',
          monto_total: propuesta.monto_total || '',
          tipo_licencia: propuesta.tipo_licencia || 'BA',
          num_usuarios: propuesta.num_usuarios || 1,
          estado: propuesta.estado || 'EN',
          observaciones: propuesta.observaciones || '',
        });

        setProspectos(prospectosResponse.data);
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.patch(`/propuestas/${id}/`, payload);

      setExito('Propuesta actualizada correctamente.');
      ...
      if (data) {
        ...
        setError(firstMessage || 'No se pudo actualizar la propuesta.');
      } else {
      ...
      <div style={cardStyle}>
        <div style={headerStyle}>
          <div>
            ...
            <h1 style={titleStyle}>Editar propuesta</h1>
            <p style={descriptionStyle}>
              Actualiza la información comercial de la propuesta seleccionada.
            </p>
          </div>
